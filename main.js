...
var DEFAULT_SETTINGS = {
  ...
  maxNodeLength: 20,
  nodeStyle: "pill",
  opmlImportCreateNotes: false,
  opmlImportSubfolder: "OPML Import"
};

// src/pro/proLogic.ts
var import_obsidian = require("obsidian");
async function nodeToOPML(node, level, app, settings, escapeXml2, prepareNoteContent2) {
  const indent = "  ".repeat(level);
  const escapedText = escapeXml2(node.text);
  let noteAttr = "";
  ...
  if (wikiLinkMatch) {
    ...
    if (file instanceof import_obsidian.TFile) {
      ...
      const cleanedContent = prepareNoteContent2(content, settings, escapeXml2);
      noteAttr = ` _note="${cleanedContent}" note="${cleanedContent}"`;
    }
  }
  if (node.children.length === 0) {
    return `${indent}<outline text="${escapedText}"${noteAttr}/>
`;
  } else {
    let res = `${indent}<outline text="${escapedText}"${noteAttr}>
`;
    for (const child of node.children) {
      res += await nodeToOPML(child, level + 1, app, settings, escapeXml2, prepareNoteContent2);
    }
    res += `${indent}</outline>
`;
    return res;
...
function prepareNoteContent(content, settings, escapeXml2) {
  if (!content)
    return "";
  let cleaned = content.replace(/\[ \]/g, "\u2610").replace(/\[[xX]\]/g, "\u2611").replace(/^(\s*[-*+]\s+)?(#+)\s+(.*)$/gm, (m, g1, g2, g3) => {
    const level = g2.length;
    let symbol = "\xB7 ";
    if (level === 1)
      symbol = "\u25A0 ";
    else if (level === 2)
      ...
      symbol = "\u25CB ";
    return (g1 || "") + "  ".repeat(level - 1) + symbol + g3;
  }).replace(/^(\s*)[-*+]\s+/gm, "$1\u2022 ").replace(/\*\*([^*]+)\*\*/g, "$1").replace(/__([^_]+)__/g, "$1").replace(/\*([^*]+)\*/g, "$1").replace(/_([^_]+)_/g, "$1").replace(/\[\[([^\]|]+)(?:\|([^\]]+))?\]\]/g, (m, g1, g2) => g2 || g1).replace(/\[([^\]]+)\]\(([^)]+)\)/g, "$1").replace(/`([^`]+)`/g, "$1").trim();
  return escapeXml2(cleaned, true);
}
function escapeXml(unsafe, preserveNewlines = false) {
...
async function exportOPML(mindMapData, app, settings) {
  const rootNode = mindMapData.root;
  const baseFileName = rootNode.text.replace(/[/\\?%*:|"<>]/g, "-").trim() || "Untitled";
  let opml = `<?xml version="1.0" encoding="UTF-8"?>
<opml version="2.0">
  <head>
    <title>${escapeXml(rootNode.text)}</title>
  </head>
  <body>
`;
  opml += await nodeToOPML(rootNode, 2, app, settings, escapeXml, prepareNoteContent);
  opml += `  </body>
</opml>`;
  ...
  a.href = url;
  a.download = `${baseFileName}.opml`;
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  URL.revokeObjectURL(url);
  new import_obsidian.Notice(`Exporting OPML...`);
}
function sanitizeFileName(name) {
  return name.replace(/[/\\?%*:|"<>]/g, "-").trim() || "Untitled";
}
function getOutlineNoteBody(outline) {
  return (outline.getAttribute("_note") || outline.getAttribute("note") || "").trim();
}
function getOpmlImportFolderPath(sourceName, settings) {
  var _a;
  const subfolder = (settings.opmlImportSubfolder || "OPML Import").trim() || "OPML Import";
  const projectName = sanitizeFileName(sourceName.replace(/\.opml$/i, ""));
  const exportFolder = ((_a = settings.exportFolder) == null ? void 0 : _a.trim()) || "";
  return exportFolder ? `${exportFolder}/${subfolder}/${projectName}` : `${subfolder}/${projectName}`;
}
...
async function importOPMLContent(content, sourceName, mindMapView, app, settings) {
  const parser = new DOMParser();
  const xmlDoc = parser.parseFromString(content, "text/xml");
  const parseError = xmlDoc.querySelector("parsererror");
  if (parseError) {
    new import_obsidian.Notice("Invalid OPML: Could not parse XML.");
    return;
  }
  const bodyOutlines = Array.from(xmlDoc.querySelectorAll("body > outline"));
  if (bodyOutlines.length === 0) {
    new import_obsidian.Notice("Invalid OPML: No outline found in body.");
    return;
  }
  const createNotes = settings.opmlImportCreateNotes;
  let importFolder = "";
  const usedNames = /* @__PURE__ */ new Set();
  let notesCreated = 0;
  if (createNotes) {
    importFolder = getOpmlImportFolderPath(sourceName, settings);
    await ensureVaultFolder(app, importFolder);
  }
  const parseOutline = async (outline) => {
    const title = outline.getAttribute("text") || outline.getAttribute("title") || "";
    const noteBody = getOutlineNoteBody(outline);
    let nodeText = title;
    if (createNotes && noteBody) {
      const stem = sanitizeFileName(title);
      let candidate = stem;
      let counter = 2;
      const nameKey = (s) => s.toLowerCase();
      while (usedNames.has(nameKey(candidate)) || await app.vault.adapter.exists(`${importFolder}/${candidate}.md`)) {
        candidate = `${stem} (${counter})`;
        counter++;
      }
      usedNames.add(nameKey(candidate));
      await app.vault.create(`${importFolder}/${candidate}.md`, noteBody);
      nodeText = `[[${candidate}]]`;
      notesCreated++;
    }
    const node = {
    ...
    for (const child of children) {
      const childNode = await parseOutline(child);
  ...
  let rootNode;
  if (bodyOutlines.length === 1) {
    rootNode = await parseOutline(bodyOutlines[0]);
  } else {
    rootNode = {
      id: "root",
      text: sourceName.replace(/\.opml$/i, ""),
      children: []
    };
    for (const outline of bodyOutlines) {
      const childNode = await parseOutline(outline);
      childNode.parent = rootNode.id;
      rootNode.children.push(childNode);
    }
  }
  rootNode.id = "root";
  mindMapView.mindMapData.root = rootNode;
  mindMapView.selectedNodeId = rootNode.id;
  mindMapView.selectedNodeIds.clear();
  mindMapView.selectedNodeIds.add(rootNode.id);
  mindMapView.render();
  await mindMapView.saveMindMap(true);
  if (createNotes) {
    new import_obsidian.Notice(`Imported ${sourceName}: ${notesCreated} note(s) in ${importFolder}`);
  } else {
    new import_obsidian.Notice(`Imported ${sourceName} (structure only)`);
  }
}
function promptOPMLImport(mindMapView, app, settings) {
  ...
  input.onchange = async (e) => {
    ...
    if (file) {
      ...
      await importOPMLContent(content, file.name, mindMapView, app, settings);
    }
  };
  input.click();
}
...
var MindMapPlugin = class extends import_obsidian2.Plugin {
  ...
  async onload() {
    ...
    this.addCommand({
      ...
      callback: () => {
        ...
        const view = leaves.length > 0 ? leaves[0].view : null;
        if (view instanceof MindMapView) {
          void view.promptOPMLImport();
        } else {
          void this.activateView().then(() => {
            const newLeaves = this.app.workspace.getLeavesOfType(VIEW_TYPE_MIND_MAP);
            const newView = newLeaves.length > 0 ? newLeaves[0].view : null;
            if (newView instanceof MindMapView)
              void newView.promptOPMLImport();
          });
        }
      }
    });
    this.registerEvent(
      this.app.vault.on("rename", (file, oldPath) => {
        if (this.isInternalRenaming)
          return;
        if (file instanceof import_obsidian2.TFile && (file.extension === "md" || file.extension === "mindmap")) {
    ...
    this.registerEvent(
      this.app.vault.on("create", (file) => {
        if (file instanceof import_obsidian2.TFile && file.extension === "mindmap") {
  ...
  async handleFileRename(file, oldPath) {
    ...
    for (const path of this.mindmapPaths) {
      ...
      if (!(mmFile instanceof import_obsidian2.TFile))
        continue;
...
\`\`\``;
    try {
      ...
      new import_obsidian2.Notice(`Error creating mind map: ${e.message}`);
    }
  }
...
var MindMapSettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian2.Setting(containerEl).setName("Export folder").setDesc("Default folder for exported files and new notes.").addText(
    ...
    new import_obsidian2.Setting(containerEl).setName("Theme").setDesc("Select a color theme for your mindmap nodes.").addDropdown(
    ...
    new import_obsidian2.Setting(containerEl).setName("OPML import").setHeading();
    new import_obsidian2.Setting(containerEl).setName("Create notes from OPML body").setDesc(
      "When enabled, Scrivener-style _note content is saved as vault Markdown files and nodes link to them. When disabled, only outline titles are imported."
    ).addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.opmlImportCreateNotes).onChange((value) => {
        this.plugin.settings.opmlImportCreateNotes = value;
        void this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("OPML import subfolder").setDesc(
      "Notes are created under Export folder / this subfolder / OPML file name. Example: OPML Import"
    ).addText(
      (text) => text.setPlaceholder("OPML Import").setValue(this.plugin.settings.opmlImportSubfolder).onChange((value) => {
        this.plugin.settings.opmlImportSubfolder = value;
        void this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Strip metadata from full note").setDesc("Automatically remove YAML frontmatter/properties when exporting content.").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.stripMetadata).onChange((value) => {
        this.plugin.settings.stripMetadata = value;
        void this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Show hover preview").setDesc("Show the full node text when hovering over truncated nodes.").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.showHoverPreview).onChange((value) => {
        this.plugin.settings.showHoverPreview = value;
        void this.plugin.saveSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Max node text length").setDesc("The maximum number of characters to show in a node before truncating with '...'.").addSlider(
    ...
    new import_obsidian2.Setting(containerEl).setName("Node operations").setHeading();
    const opsSection = containerEl.createDiv();
    const createOpsHotkey = (name, key) => {
      new import_obsidian2.Setting(opsSection).setName(name).addText((text) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Node style").setDesc("Choose the visual appearance of nodes.").addDropdown((dropdown) => dropdown.addOption("pill", "Pill (rounded)").addOption("rect", "Rectangle (sharp)").setValue(this.plugin.settings.nodeStyle).onChange((value) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Navigation and movement").setHeading();
    ...
    new import_obsidian2.Setting(moveSection).setName("Multi-node selection").setDesc("Modifier key to select multiple nodes during navigation.").addText((text) => text.setDisabled(true).setValue("Cmd / Ctrl (fixed)"));
    const createMoveHotkey = (name, key) => {
      new import_obsidian2.Setting(moveSection).setName(name).addText((text) => {
        text.setValue(this.plugin.settings.hotkeys[key]);
        text.inputEl.placeholder = "Press keys...";
        text.inputEl.addEventListener("keydown", (e) => {
          e.preventDefault();
...
var SaveModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian2.Setting(contentEl).setName("Save mind map").setHeading();
    new import_obsidian2.Setting(contentEl).setName("File name").addText(
...
var _MindMapView = class extends import_obsidian2.TextFileView {
...
\`\`\``;
  ...
  async onOpen() {
    ...
    const toggleBtn = this.createControlButton(controls, "lucide-menu", "Menu", () => {
      ...
      (0, import_obsidian2.setIcon)(toggleBtn, this.isToolbarExpanded ? "lucide-chevron-left" : "lucide-menu");
    });
    ...
    this.createControlButton(btnContainer, "lucide-download", "Import OPML", () => void this.promptOPMLImport());
    this.createControlButton(btnContainer, "lucide-upload", "Export OPML", () => void this.exportOPML());
  ...
  createControlButton(container, iconId, tooltip, onClick) {
    ...
    (0, import_obsidian2.setIcon)(btn, iconId);
    (0, import_obsidian2.setTooltip)(btn, tooltip);
  ...
  async exportFullNote() {
    ...
    try {
      ...
      new import_obsidian2.Notice(`Full mind map exported: ${path}`);
      ...
      new import_obsidian2.Notice(`Failed to export full note: ${e.message}`);
    }
  }
...
`;
  ...
  async exportMarkdown() {
    ...
    try {
      await this.app.vault.create(path, markdown);
      new import_obsidian2.Notice(`Mind map exported as a new file: ${path}`);
      const newFile = this.app.vault.getAbstractFileByPath(path);
      if (newFile instanceof import_obsidian2.TFile) {
        await this.app.workspace.getLeaf("tab").openFile(newFile);
      }
    } catch (e) {
      new import_obsidian2.Notice(`Failed to export: ${e.message}`);
      console.error(e);
    }
  }
...
`;
  ...
  async promptOPMLImport() {
    await promptOPMLImport(this, this.app, this.settings);
  }
  async exportOPML() {
    await exportOPML(this.mindMapData, this.app, this.settings);
  }
  async importOPML(file) {
    const targetFile = file || this.app.workspace.getActiveFile();
    if (!targetFile)
      return;
    if (targetFile.extension !== "opml" && !file) {
      new import_obsidian2.Notice("Please select an OPML file.");
      return;
    }
    const content = await this.app.vault.read(targetFile);
    await importOPMLContent(content, targetFile.name, this, this.app, this.settings);
  }
  ...
  handleExternalDrop(e, node, dropMode) {
    ...
    if (dragManager) {
      ...
      if (context && context.file instanceof import_obsidian2.TFile) {
        file = context.file;
      } else if (dragManager.draggable && dragManager.draggable.file instanceof import_obsidian2.TFile) {
  ...
  undo() {
    ...
    if (prevState) {
      ...
      this.render();
      void this.saveMindMap(true);
      new import_obsidian2.Notice("Undo");
    }
  }
  async saveMindMap(isAutoSave = false) {
    if (this.file) {
      this.requestSave();
      if (!isAutoSave)
        new import_obsidian2.Notice("Mind map saved.");
      return;
    }
    if (!isAutoSave) {
      const rootNode = this.mindMapData.root;
  ...
  async performSave(fileName, isAutoSave) {
    ...
    try {
      ...
      if (!isAutoSave) {
        new import_obsidian2.Notice(`Mind map saved to ${path}`);
        const newFile = this.app.vault.getAbstractFileByPath(path);
        if (newFile instanceof import_obsidian2.TFile) {
      ...
      if (!isAutoSave)
        new import_obsidian2.Notice(`Error saving: ${e.message}`);
    }
  }
  render() {
          ...
          if (file instanceof import_obsidian2.TFile) {
            ...
            try {
              ...
              if (newPath !== file.path) {
                ...
                if (destinationFile) {
                  new import_obsidian2.Notice(`Cannot rename note: "${newFileName}.md" already exists.`);
                } else {
              ...
              if (!e.message.includes("already exists")) {
                new import_obsidian2.Notice(`Failed to rename linked note: ${e.message}`);
              }
            } finally {
              setTimeout(() => {
  ...
  async createNoteFromNode() {
    ...
    try {
      ...
      if (file instanceof import_obsidian2.TFile) {
        ...
        new import_obsidian2.Notice(`New note created and linked: ${file.basename}`);
      }
    } catch (e) {
      new import_obsidian2.Notice(`Error creating note: ${e.message}`);
    }
  }
  renameFile() {
    if (!this.file)
  ...
  async openLinkedNote() {
    ...
    if (wikiLinkMatch) {
      ...
      if (file instanceof import_obsidian2.TFile) {
        ...
        new import_obsidian2.Notice(`File not found: ${fileName}`);
      }
    } else {
      new import_obsidian2.Notice("No linked note found in this node.");
    }
  }
};
...
var RenameModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    const { contentEl } = this;
    new import_obsidian2.Setting(contentEl).setName("Rename mind map").setHeading();
    new import_obsidian2.Setting(contentEl).setName("New name").addText(
